// 服务端配置与启动参数：解析命令行/环境变量/.env 合并出
// ServerConfig（监听、TLS、日志、数据库、WebSocket、ML、腾讯地图等）。 关键约束：allowInsecureHttp
...
// 环境变量读取抽象（可注入测试桩）；进程环境变量优先于 .env 文件值（.env 加载细节见
// server_config_environment.h）。
