...
{
  public:
    Stmt(QSqlDatabase* database, const char* sql) : query_(*database)
    {
    ...
    {
        query_.bindValue(QStringLiteral(":p%1").arg(index), QVariant::fromValue<qlonglong>(value));
    }

    void bind(const int index, const std::string_view value)
    {
        query_.bindValue(QStringLiteral(":p%1").arg(index),
                         QString::fromUtf8(value.data(), static_cast<qsizetype>(value.size())));
    }

    void bindNull(const int index)
    {
        query_.bindValue(QStringLiteral(":p%1").arg(index), QVariant());
    }
    ...
    {
        const auto cell = query_.value(column).toString().toUtf8();
        return {cell.constData(), static_cast<std::size_t>(cell.size())};
    }
  ...
  private:
    ...
    {
        throw std::runtime_error(queryError(query_));
    }
...
int64_t maxId(QSqlDatabase* database, const char* table)
{
...
void removeLegacyStations(QSqlDatabase* database)
{
    ...
    Stmt find(database, "SELECT id FROM station WHERE code=:p1");
    Stmt references(database, "SELECT (SELECT COUNT(*) FROM charging_flow WHERE station_id=:p1)"
                              "+(SELECT COUNT(*) FROM charging_order WHERE station_id=:p1)"
                              "+(SELECT COUNT(*) FROM flow_queue WHERE station_id=:p1)"
                              "+(SELECT COUNT(*) FROM price_adjustment WHERE station_id=:p1)"
                              "+(SELECT COUNT(*) FROM station_hourly_metric WHERE station_id=:p1)"
                              "+(SELECT COUNT(*) FROM load_prediction WHERE station_id=:p1)"
                              "+(SELECT COUNT(*) FROM device_command WHERE charger_id IN"
                              "(SELECT id FROM charger WHERE station_id=:p1))");
    Stmt deleteChargers(database, "DELETE FROM charger WHERE station_id=:p1");
    Stmt deleteStation(database, "DELETE FROM station WHERE id=:p1");
...
void reconcileFleet(QSqlDatabase* database, std::vector<ChargerRef> pools[kSiteCount][2],
                    int64_t siteStationIds[kSiteCount])
{
    removeLegacyStations(database);
    int64_t stationId = maxId(database, "station") + 1;
    Stmt findStation(database, "SELECT id FROM station WHERE code=:p1");
    Stmt insertStation(
        database, "INSERT INTO station(id,code,name,address,adcode,latitude_e6,longitude_e6,"
                  "business_hours,enabled) VALUES(:p1,:p2,:p3,:p4,:p5,:p6,:p7,'00:00-24:00',1) "
    ...
    Stmt insertCharger(database, "INSERT INTO charger(id,station_id,code,charger_type,power_watt,"
                                 "connector_standard,status) VALUES(:p1,:p2,:p3,:p4,:p5,:p6,0) "
    ...
    Stmt tariffInsert(database, "INSERT INTO region_tariff(adcode,electricity_cent_per_kwh,"
    ...
    Stmt pool(database, "SELECT id,power_watt,charger_type,code FROM charger WHERE station_id=:p1 "
...
void validateSeedConflicts(QSqlDatabase* database)
{
    Stmt find(database, "SELECT username,phone FROM user_account WHERE username=:p1 OR phone=:p2 "
                        "LIMIT 1");
...
void applyFullDemoSeed(QSqlDatabase* database, const int64_t anchorAt)
{
    ...
    Stmt insertUser(
        ...
        "debt_cent,has_active_flow,version,deleted) VALUES(:p1,:p2,:p3,1,:p4,:p5,0,0,1,0) "
        "RETURNING id");
    for (int owner = 0; owner < kOwnerCount; ++owner)
    {
        char username[24];
        ...
        if (!insertUser.step())
            throw std::runtime_error("seed user insert did not return an id");
        ownerIds[owner] = insertUser.integer(0);
    }
    Stmt insertFlow(database,
    ...
    Stmt insertOrder(
        ...
        " VALUES(:p1,:p2,:p3,:p4,:p5,:p6,:p7,:p8,:p9,:p10,:p11,:p12,NULL,:p13,:p14,:p15,:p16,:p17,:"
        "p18,:p19,0,"
        ":p20,0,:p21)");
    ...
    Stmt insertWallet(database, "INSERT INTO wallet_account(user_id,balance_cent,debt_cent,version,"
                                "updated_at) VALUES(:p1,:p2,0,:p3,:p4)");
    ...
    Stmt insertRecharge(database,
                        ...
                        "completed_at) VALUES(:p1,:p2,:p3,0,:p3,:p4,0,:p5)");
    Stmt updateUserBalance(database, "UPDATE user_account SET balance_cent=:p1 WHERE id=:p2");
    for (int owner = 0; owner < kOwnerCount; ++owner)
    {
    ...
    Stmt updateCharger(database, "UPDATE charger SET total_count=:p1,total_minutes=:p2 WHERE "
                                 "id=:p3");
