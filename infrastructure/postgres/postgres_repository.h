// PostgresRepository：平台唯一的 PostgreSQL 持久化适配器（物理模型见 docs/database-design.md）。
...
class PostgresRepository final : public database::PlatformRepository
{
  public:
    explicit PostgresRepository(PostgresConfig config);
    ...
    std::optional<core::application::UserAccount> findById(std::int64_t id) const override;
    ...
    core::application::AccountWriteResult create(core::application::UserAccount& account) override;
    core::application::AccountWriteResult
    updateNickname(std::int64_t id, std::int64_t expectedVersion, std::string nickname,
                   core::application::UserAccount& updated) override;
    core::application::AccountWriteResult
    updateStatus(std::int64_t id, int status, core::application::UserAccount& updated) override;
    core::application::AccountWriteResult
    updateCredential(std::int64_t id, std::string username, std::string passwordHash,
                     core::application::UserAccount& updated) override;
    core::application::AccountWriteResult
    ...
    void addWalletTransaction(const core::application::WalletTransaction& transaction) override;
    std::vector<core::application::WalletTransaction>
    walletTransactions(std::int64_t userId,
    ...
    std::optional<core::application::Station> station(std::int64_t stationId) override;
    ...
    std::optional<core::application::Charger> charger(std::int64_t chargerId) override;
    void saveCharger(const core::application::Charger& charger) override;
    std::optional<core::application::RegionTariff> effectiveTariff(const std::string& adcode,
    ...
    std::optional<core::application::ChargingFlow> flow(const std::string& flowNo) override;
    std::optional<core::application::ChargingFlow> activeFlow(std::int64_t userId) override;
    std::vector<core::application::ChargingFlow> flowsWithStatus(int status) override;
    void addFlowEvent(const core::application::FlowEvent& event) override;
    void addChargerStatusEvent(const core::application::ChargerStatusEvent& event) override;
    std::vector<core::application::OutboxEvent> pollOutbox(std::int64_t now, int limit) override;
    ...
    std::vector<core::application::ChargingOrder> orders(std::int64_t userId,
    ...
    void refreshReadiness() override;
    std::int64_t nextBusinessSequence(std::string_view prefix, std::int64_t utcDay) override;
    ...
    void
    saveIdempotencyRecord(const core::application::PersistedIdempotencyRecord& record) override;
    void removeIdempotencyRecord(std::string_view scope, std::string_view key) override;
    void cleanupIdempotencyRecords(std::int64_t now) override;
    std::size_t idempotencyRecordCount() override;

    void ensureDevelopmentAdmin(bool enabled) override;
    ...
    core::application::AdminAccountPage
    adminAccounts(const core::application::AdminAccountQuery& query) override;
    core::application::AdminAccountWriteResult
    createAdminAccount(std::int64_t actorAdminId, std::string_view username,
                       std::string_view passwordHash, std::string_view reason, std::int64_t at,
                       core::application::AdminAccount& created) override;
    core::application::AdminAccountWriteResult
    updateAdminAccountStatus(std::int64_t actorAdminId, std::int64_t adminId, int status,
                             ...
                             core::application::AdminAccount& updated) override;
    core::application::AdminAccountWriteResult
    changeAdminAccountPassword(std::int64_t actorAdminId, std::int64_t adminId,
                               std::string_view expectedCurrentHash,
                               std::string_view newPasswordHash, std::int64_t at,
                               core::application::AdminAccount& updated) override;
    core::application::AdminUserPage
    listManagedUsers(const core::application::AdminUserQuery& query) override;
    std::optional<core::application::UserAccount> findManagedUser(std::int64_t id) override;
    core::application::AccountWriteResult
    updateManagedUserStatus(std::int64_t actorAdminId, std::int64_t userId, int status,
                            std::string_view reason, std::int64_t expectedVersion, std::int64_t at,
                            core::application::UserAccount& updated) override;
    ...
    bool createStationWithChargers(core::application::Station& station,
                                   const core::application::InitialChargerSpec& spec) override;
    std::vector<core::application::Station> stations(std::optional<int> status,
                                                     std::optional<std::string> adcode,
                                                     const std::string& keyword) override;
    bool addChargers(std::vector<core::application::Charger>& chargers) override;
    void addTariffVersion(const core::application::RegionTariff& tariff) override;
    bool tariffOverlaps(const std::string& adcode, std::int64_t from, std::int64_t to) override;
    std::int64_t addPriceAdjustment(const core::application::PriceAdjustment& adjustment) override;
    std::optional<core::application::PriceAdjustment>
    effectivePriceAdjustment(std::int64_t stationId, int chargerType, std::int64_t at) override;
    void addDeviceCommand(const core::application::DeviceCommand& command) override;
    std::optional<core::application::DeviceCommand>
    deviceCommand(const std::string& commandNo) override;
    void saveDeviceCommand(const core::application::DeviceCommand& command) override;
    std::vector<core::application::DeviceCommand> dueDeviceCommands(std::int64_t now) override;
    ...
    core::application::AdminFlowPage flows(const core::application::AdminFlowQuery& query) override;
    ...
    std::optional<core::application::MlTask> runningMlTask(const std::string& taskType) override;
    std::vector<core::application::MlTask> overdueMlTasks(std::int64_t trainDeadline,
                                                          std::int64_t predictDeadline) override;
    void addMlTask(const core::application::MlTask& task) override;
    std::optional<core::application::MlTask> mlTask(const std::string& taskNo) override;
    void saveMlTask(const core::application::MlTask& task) override;
    bool tryFinishMlTask(const core::application::MlTask& task, bool allowTimedOut) override;
    ...
    std::optional<core::application::BackupRecord> backup(const std::string& backupNo) override;
    void saveBackup(const core::application::BackupRecord& record) override;
    bool createBackupSnapshot(core::application::BackupRecord& record) override;
    bool verifyBackupSnapshot(const core::application::BackupRecord& record) override;
    void cleanupAdminRecords(std::int64_t now) override;
    void refreshHourlyMetrics(std::int64_t fromAt, std::int64_t toAt) override;
    core::application::HourlyMetricPage hourlyMetrics(std::int64_t fromAt, std::int64_t toAt,
                                                      std::optional<std::int64_t> stationId,
                                                      std::string_view cursor, int limit) override;
    std::int64_t nextDashboardVersion() override;
    void addModelVersion(const core::application::ModelVersion& version) override;
    std::optional<core::application::ModelVersion>
    modelVersion(std::string_view versionNo) override;
    std::optional<core::application::ModelVersion> latestQualifiedModel() override;
    void upsertPredictions(const std::vector<core::application::LoadPrediction>& items) override;
    std::vector<core::application::LoadPrediction>
    predictions(std::optional<std::int64_t> stationId, std::optional<int> horizonHour,
                std::int64_t fromAt) override;
    void markPredictionsStale() override;
    void cleanupAnalytics(std::int64_t now) override;
    const PostgresConfig& config() const
    {
        return config_;
    }

    QSemaphore& connectionSlots() const
    {
        return connectionSlots_;
    }
