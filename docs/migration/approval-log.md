...
| A-04 | 钱包、账务和支付 | APPROVED | `c54c303`；四端点契约、退款语义与 WalletStore 已审查通过，B-07 持久化收口已合入集成线 |
...
| A-06 | 管理端扩展业务 | APPROVED | `133a419`；六项修复、真实 PG 集成测试及 10 个端点契约登记通过（审批 `9092787`；与 A-05 线 `935801e` 同内容并行交付，管理端代码逐字节一致，已随 PR #40 在 develop） |
